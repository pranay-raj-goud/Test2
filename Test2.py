...
def process_data(uploaded_file, partner_id, buffer_percent, grade, district_digits, block_digits, school_digits, student_digits, selected_param):
    ...
    data['Partner_ID'] = str(partner_id).zfill(len(str(partner_id)))
    data['Grade'] = grade
...
def main():
    ...
    if uploaded_file is not None:
        ...
        # Display the ID format based on user inputs
        id_format = generate_id_format_string(selected_param, school_digits, grade_digits, student_digits)
        st.markdown(f"<p style='font-size: large;'>Your ID would be: {id_format}</p>", unsafe_allow_html=True)
